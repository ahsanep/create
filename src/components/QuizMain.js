...
export default class Quiz extends Component {
  ...
  checkAnswer = (answer) => {
    ...
    if (answer === correctAnswers[step]) {
      const total = Object.keys(this.state.quiestions).length;
  ...
  render() {
    ...
    return (
      <div className="Content">
        <div
          className="w3-grey"
          style={{ height: '24px', width: this.state.percent + '%' }}
        ></div>
        {step <= Object.keys(quiestions).length ? (
          <>
            <Question
              ...
              step={step}
              total={Object.keys(quiestions).length}
            />
            <Answer
              answer={answers[step]}
              step={step}
            ...
            <h1> Score: {this.state.scorePercent}%</h1>
            {/* <h1>
              {' '}
              Min Score:{' '}
              {((Object.keys(quiestions).length -
                (this.state.score - this.state.step)) /
                Object.keys(quiestions).length) *
                100}
              %
            </h1>
            <h1>
              ...
              Max Score:{' '}
              {((Object.keys(quiestions).length -
                (this.state.step - this.state.score - 1)) /
                Object.keys(quiestions).length) *
                100}{' '}
              %
            </h1> */}
